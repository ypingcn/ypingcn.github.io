...
// __sighandler_t　是指向函数的指针，只有一个整型参数且无返回值
...
// 定时产生SIGALRM　信号，返回值是0或者之前设置的剩余时间
int pause(void)
...
#include <signal.h>
int sigprocmask(int __how, const sigset_t *__restrict __set, sigset_t *__restrict __oset) // 信号屏蔽集内的信号的处理方式，单线程用
// how
SIG_BLOCK //加入信号到进程屏蔽。
...
#include <signal.h> // 恢复信号屏蔽字，再使进程休眠
int sigsuspend(const sigset_t *__set) // 返回-1 errno==EINTR
...
＃include <unistd.h>
unsigned int sleep(unsigned int __seconds) // 返回０或未休眠完的秒数
// 调用者被挂起直到在时间到了或者捕捉到一个信号并处理完返回

#include <time.h>
int nanosleep(const struct timespec *__requested_time, struct timespec *__remaining)
// 参数为需要休眠的时间、未休眠的时间
int clock_nanosleep(clockid_t __clock_id, int __flags, const struct timespec *__req, struct timespec *__rem)// 正常返回0 错误返回错误码
// flags 为0相对时间　TIMER_ABSTIME为绝对
```