...
//openat 的第一个参数可以用 open 返回值，例如

cout<<openat(open("/home/",O_DIRECTORY),"a.py",O_RDONLY)<<endl;
...
//返回新的成功偏移量，错误-1
//- __whence
//SEEK_SET SEEK_CUR SEEK_END 开始位置　当前位置　结束位置
...
#include <unistd.h>
void sync() //将修改过的块排入队列并马上返回
int fsync(int __fd) //特定文件操作，更新数据＋属性，磁盘写完成后才返回。0 正常-1　错误。
int fdatasync(int __fildes) //类似fsync ，但只更新数据
